# CodeStat - Git 代码提交统计工具

一个高效的 Git 代码提交统计工具，支持批量统计多个仓库的提交记录，帮助开发者跟踪工作进度和代码贡献。

## 功能特性

- 📊 **批量统计**：自动扫描并统计多个 Git 仓库的提交记录
- 👤 **个人贡献追踪**：区分统计当前用户与其他用户的提交
- 📅 **灵活的时间范围**：支持昨天、今天或指定任意日期
- 🎯 **工作日检查**：自动识别工作日与周末，设置不同的代码量标准
- ⚙️ **可配置**：支持通过配置文件自定义扫描目录、深度和代码量标准
- 📈 **可视化报告**：提供清晰的统计报告和完成进度

## 安装使用

### 快速开始

1. 克隆或下载脚本：
```bash
git clone <repository_url>
cd CodeStat
```

2. 赋予执行权限：
```bash
chmod +x statistics.sh
```

3. 直接运行（统计昨天的提交）：
```bash
./statistics.sh
```

### 使用示例

```bash
# 统计昨天的提交（默认）
./statistics.sh

# 统计今天的提交
./statistics.sh today

# 统计指定日期的提交
./statistics.sh 2026-01-19
```

## 配置说明

在脚本同目录下创建 `.statistics.conf` 文件可自定义配置：

```bash
# 扫描目录（绝对路径或相对路径）
SCAN_DIR="/path/to/projects"

# 扫描深度（递归查找 git 仓库的层数）
SCAN_DEPTH=3

# 工作日代码量标准（行/天）
DAILY_CODE_STANDARD=500
```

### 配置项说明

- **SCAN_DIR**：指定扫描的根目录，脚本会递归查找该目录下的所有 Git 仓库
- **SCAN_DEPTH**：控制递归查找的深度，避免扫描过深
- **DAILY_CODE_STANDARD**：工作日每天期望的代码量标准，周末不强制要求

## 统计输出示例

```
统计日期: 2026-01-19 (昨天)
统计用户: John Doe
扫描目录: /path/to/projects
扫描深度: 3 层
日期类型: 工作日（代码量标准: 500 行/天）

=== 正在统计: project-1 ===
  文件变更: 12 (我的: 8)
  新增行数: 256 (我的: 180)
  删除行数: 45 (我的: 30)
  净增行数: 211 (我的: 150)

==========================================
统计汇总 (2026-01-19)
==========================================
仓库数量: 3
------------------------------------------
所有人汇总:
  文件变更总数: 25
  新增行数总计: 520
  删除行数总计: 80
  净增行数总计: 440
------------------------------------------
John Doe 的贡献:
  文件变更数: 15
  新增行数: 320
  删除行数: 50
  净增行数: 270
  占比: 61.5%
==========================================

==========================================
工作日代码量检查 (昨天)
==========================================
标准要求: 每天 500 行代码
实际提交: 320 行
完成进度: 64.0%

⚠️  警告: 昨天未达到工作日代码量标准！
⚠️  还需要补充: 180 行代码
⚠️  请努力赶上进度！
```

## 工作原理

1. 扫描指定目录下所有 `.git` 文件夹（递归深度可配置）
2. 使用 `git log` 命令统计指定日期范围内的提交记录
3. 解析提交统计信息（新增行数、删除行数、文件变更数）
4. 分别统计所有用户和当前 git 用户（通过 `git config user.name`）的贡献
5. 汇总生成报告，并根据工作日标准进行进度检查

## 系统要求

- **操作系统**：Linux 或 macOS
- **依赖工具**：
  - Bash shell
  - Git
  - awk、date 等标准 Unix 工具

## 注意事项

- 脚本会根据当前 git 配置（`git config user.name`）识别个人提交
- 配置文件优先级：配置文件 > 默认值
- 周末（周六、周日）不进行代码量告警
- 日期格式必须为 `YYYY-MM-DD`

## 许可证

MIT License

## 贡献

欢迎提交 Issue 和 Pull Request！
